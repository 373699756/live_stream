...
export function startHlsPreview({
    ...
    const warmup = () => {
        ...
        void fetch(hlsSessionUrl, {
            ...
            if (response.ok && hlsPlaylistHasSegment(await response.text())) {
                launchHlsPlayer();
                return;
            }
            if (window.performance.now() - startedAt >= hlsStartupTimeoutMs) {
    ...
    return window.setTimeout(() => {
        ...
        fallbackFromHlsFailure(hlsPlayerLaunched ? 'HLS 播放超时' : 'HLS 启动超时');
