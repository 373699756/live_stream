...
StreamMjpegClientId MjpegClientRegistry::Attach(
    StreamId stream_id, IStreamMjpegSink *sink, size_t max_clients) {
    if (sink == nullptr || mjpeg_clients_.size() >= max_clients) {
        return 0;
    }
    const StreamMjpegClientId client_id = next_mjpeg_client_id_++;
...
bool MjpegClientRegistry::Detach(StreamMjpegClientId client_id) {
...
void MjpegClientRegistry::ReleaseWrite(StreamMjpegClientId client_id) {
    auto iter = mjpeg_clients_.find(client_id);
    if (iter == mjpeg_clients_.end()) {
        return;
    }
...
bool MjpegClientRegistry::EraseDetachedClient(StreamMjpegClientId client_id,
                                              MjpegClientState *client) {
