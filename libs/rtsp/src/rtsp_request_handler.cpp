...
void RtspRequestHandler::HandleRequest(
    ...
    StreamId stream_id = rtsp_session.stream_id;
    if ((request.method == "DESCRIBE" || request.method == "SETUP") &&
        ...
        SendResponse(rtsp_session.connection_id, 404, request, {}, "");
        return;
    }
    if ((request.method == "DESCRIBE" || request.method == "SETUP") &&
    ...
    if (!delegate_.AuthorizeRtspRequest(rtsp_session, request, stream_id)) {
        return;
    }

    if (request.method == "DESCRIBE") {
        HandleDescribe(rtsp_session, request, stream_id);
        return;
    }
    if (request.method == "SETUP") {
        (void)delegate_.SetupRtspTransport(rtsp_session, request, stream_id);
        return;
    }
    ...
    if (request.method == "TEARDOWN") {
        HandleTeardown(rtsp_session, request);
        return;
    }

    SendResponse(rtsp_session.connection_id, 455, request, {}, "");
}
