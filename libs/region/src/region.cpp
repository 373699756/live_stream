...
void RegionOverlay::RefreshMediaChannels() {
    if (options.device_media == nullptr) {
        return;
    }
    const MediaChannels current_channels = options.device_media->GetChannels();
...
bool Region::Start() {
    ...
    if (impl_->options.config != nullptr) {
        ConfigJson overlay_config =
            impl_->options.config->GetValue("overlay");
...
const char *Region::StaticName() { return "region"; }

bool Region::BindMedia(const MediaChannels &channels) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
RegionId Region::CreateRegion(const RegionConfig &config) {
    if (impl_ == nullptr) {
        return RegionId{};
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
bool Region::Attach(RegionId id) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
bool Region::Detach(RegionId id) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
bool Region::SetVisible(RegionId id, bool visible) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
bool Region::UpdateBitmap(RegionId id, const RegionBitmap &bitmap) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
bool Region::DestroyRegion(RegionId id) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
uint32_t Region::RegionCount() const {
    if (impl_ == nullptr) {
        return 0;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
RegionStats Region::GetStats() const {
    if (impl_ == nullptr) {
        return RegionStats{};
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
    RegionStats stats = impl_->stats;
