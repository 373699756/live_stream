...
constexpr const char* kPackageHelperName = "live_sysupgrade";
...
bool StartSystemUpgradeHelper(const ParsedUpgradePackage& package,
    ...
    std::string extract_reason;
    if (!ExtractUpgradeFile(package.package_path, kPackageHelperName,
                            helper_path, &extract_reason)) {
        AppendUpgradeLog("extract package helper failed: " + extract_reason);
