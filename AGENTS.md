...
- 命名必须直指业务事实。变量名应说明“是什么/为什么存在”，例如
  ...
  避免 `was_started`、`previous_state`、`target` 这类离开局部上下文就看不懂的名字。
- 命名必须精简、通俗、符合行业标准。优先为读代码的人服务，让名字直接说明用途、
  语义、角色或数据边界；不要为了显得抽象、通用或高级而起绕口名字。
- 命名长短按作用域决定：局部临时变量可以短，跨函数、跨文件、public API、配置字段和协议字段必须更明确。
  短名不能牺牲语义；长名不能重复上下文里已经清楚的信息。
- 类型和对象名优先用名词或名词短语；有副作用的函数用动词或动词短语；布尔值应能读成判断句；
  计数、大小、时间、索引、偏移等必须带清楚的单位或边界语义。
- 协议、媒体、网络、硬件等领域代码优先使用行业通用术语和标准字段名；缩写只允许使用业内常见缩写，
  不使用项目外人员看不懂的自造缩写、拼音缩写或随手省字母的写法。
- 避免 `item`、`object`、`target`、`store`、`collector`、`manager`、`helper`、`utils`
  等离开局部上下文就不清楚的泛名；确有业务语义时，名字必须补足所属对象、资源或职责。
- 已有名字准确、短、符合上下文时应保留；只有语义错误、职责变化或歧义明显时才改名。
  命名调整必须局部、必要、可解释，不做无收益的批量改名。
...
- 协议、拉流、播放兼容性或产品功能故障必须优先定位根因并修复；禁止用禁用协议、禁用功能、强制降级、
  强制绕行或隐藏入口来冒充修复。临时规避方案只能作为明确标注的 workaround 提出，并且必须先得到用户同意。
