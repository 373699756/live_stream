...
TcpSession::TcpSession(SocketIoImpl *socket_io,
                       std::shared_ptr<EventLoop> loop, int fd,
                       ConnectionId id, const TcpListenOptions &options,
                       TcpCallbacks callbacks, SocketAddress local,
                       SocketAddress peer)
...
bool TcpSession::SendSlices(const SocketWriteSlices &slices) {
...
SocketConnectionInfo TcpSession::GetInfo() const {
    std::lock_guard<std::mutex> lock(mutex_);
...
void TcpSession::HandleRead() {
    ...
    while (true) {
        ...
        if (n > 0) {
            ...
            socket_io_->AddRead(static_cast<size_t>(n));
            socket_io_->DispatchRead(callbacks_, id_, buffer, static_cast<size_t>(n));
            continue;
        }
        if (n == 0) {
...
void TcpSession::HandleWrite() {
    while (true) {
        ssize_t n = 0;
        {
            ...
            iovec iov[kMaxSocketWriteSlices];
            size_t iov_size = 0;
            for (size_t i = current.current_slice;
                 i < current.slice_size && iov_size < kMaxSocketWriteSlices;
                 ++i) {
        ...
        if (n > 0) {
            socket_io_->AddWrite(static_cast<size_t>(n));
            continue;
        }
        if (n < 0 && errno == EINTR) {
            continue;
...
void TcpSession::CheckTimeouts() {
    ...
    bool should_close = false;
    {
        ...
        // 或 RTSP interleaved 慢客户端，关闭原因会进入 socket_io connection info。
...
bool TcpSession::BuildOutBuffer(const SocketWriteSlices &slices,
                                OutBuffer *buffer) const {
    if (buffer == nullptr || slices.slice_size > kMaxSocketWriteSlices) {
        return false;
    }
    *buffer = OutBuffer{};
    size_t total_size = 0;
    for (size_t i = 0; i < slices.slice_size; ++i) {
        const SocketWriteSlice &input = slices.slices[i];
        if (input.size == 0) {
            continue;
        }
        if (input.data == nullptr || buffer->slice_size >= kMaxSocketWriteSlices ||
