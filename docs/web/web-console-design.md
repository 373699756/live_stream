...
AI 配置页暴露四类任务独立开关和全局灵敏度，不在普通界面展示模型路径、码流、
推理间隔、输入尺寸或结果数；提交时由前端按任务开关自动派生后端 `ai.enabled`
运行闸门，并将隐藏运行参数保存为一份共享配置。周界 `perimeter_regions` 通过
实时视频上的矩形绘制编辑；周界命中和告警联动都由后端 `ai` 模块判断。
