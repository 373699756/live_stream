...
bool IsValidSize(const Size &size) { return size.width > 0 && size.height > 0; }
...
bool StartSnapshotPipe(const SnapshotConfig &config) {
  SNAP_ATTR_S attr{};
...
JpegFrame ReadSnapshotStream(const SnapshotConfig &config) {
  (void)config;
  return JpegFrame{};
}
#endif
...
JpegFrame MakeHostJpeg(const SnapshotConfig &config) {
  static const uint8_t kMinimalJpeg[] = {
      ...
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xdb, 0x00, 0x43, 0x01, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      ...
      };
  std::shared_ptr<IMediaBufferPool> pool = CreateFixedMediaBufferPool(
      EstimateJpegBlockSize(config), kDefaultJpegPoolBlocks);
  if (!pool) {
    return JpegFrame{};
  }
  std::shared_ptr<IMediaBuffer> buffer = pool->Acquire();
...
JpegFrame DefaultHisiSdk::CaptureJpeg(const SnapshotConfig &config) {
  if (!IsValidSize(config.size) || config.timeout_ms == 0) {
    return JpegFrame{};
  }
#ifdef LIVE_STREAM_ENABLE_HISI_MPP
  if (!StartSnapshotPipe(config)) {
    CleanupSnapshotPipe(config);
    return JpegFrame{};
  }
  JpegFrame frame = ReadSnapshotStream(config);
  CleanupSnapshotPipe(config);
  return frame;
#else
