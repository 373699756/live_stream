...
export function ModuleStatusPanel({ status }: SystemStatusPanelsProps) {
  ...
  const extraModules = status.modules.filter((module) => !groupedNames.has(module.name));
      ...
      <div className="module-group-grid">
        {moduleGroups.map((group) => {
          const modules = group.names
            .map((name) => status.modules.find((module) => module.name === name))
            .filter((module): module is SystemStatus['modules'][number] => Boolean(module));
          if (modules.length === 0) {
            return null;
          }
          return (
