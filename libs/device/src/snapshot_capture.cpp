...
enum class SnapshotCaptureState {
    kCreated = 0,
    kInitialized,
    kStarted,
    kStopped,
...
bool ParseSnapshotCaptureConfig(const ConfigJson &value, bool *enabled,
    ...
    return json_utils::ReadField(value, "enabled", enabled) &&
           ...
           json_utils::ReadField(value, "timeout_ms", timeout_ms, 1,
                                 0xffffffffU);
}

}  // namespace

struct SnapshotCapture::Impl {
    explicit Impl(const SnapshotCaptureOptions &service_options)
    ...
    SnapshotCaptureState state = SnapshotCaptureState::kCreated;
    ...
    bool Prepare() {
        ...
        if (state == SnapshotCaptureState::kInitialized ||
            state == SnapshotCaptureState::kStarted ||
            state == SnapshotCaptureState::kStopped) {
            return true;
        }
        state = SnapshotCaptureState::kInitialized;
        return true;
    }
    ...
    void Release() {
        bool detach = false;
        {
            ...
            if (state != SnapshotCaptureState::kCreated) {
                state = SnapshotCaptureState::kDeinitialized;
            }
        }
...
SnapshotCapture::SnapshotCapture(const SnapshotCaptureOptions &options)
    : impl_(new Impl(options)) {}

SnapshotCapture::~SnapshotCapture() { Release(); }

void SnapshotCapture::Release() {
    if (impl_) {
        impl_->Release();
    }
}

bool SnapshotCapture::Start() {
    if (impl_ == nullptr) {
        return false;
    }
    bool need_init = false;
    {
        ...
        need_init = impl_->state == SnapshotCaptureState::kCreated ||
                    impl_->state == SnapshotCaptureState::kDeinitialized;
    }
    ...
    std::lock_guard<std::mutex> lock(impl_->mutex);
    if (impl_->state == SnapshotCaptureState::kStarted) {
        return true;
    }
    if (impl_->state == SnapshotCaptureState::kStopped) {
        impl_->state = SnapshotCaptureState::kInitialized;
    }
    if (impl_->state != SnapshotCaptureState::kInitialized) {
        return false;
    }
...
void SnapshotCapture::Stop() {
    ...
    if (impl_->state == SnapshotCaptureState::kStarted) {
        impl_->state = SnapshotCaptureState::kStopped;
    }
}

bool SnapshotCapture::BindMedia(const MediaChannels &channels) {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
    if (impl_->state == SnapshotCaptureState::kStarted || impl_->capturing) {
        return false;
    }
    if (!IsValidMedia(channels)) {
        return false;
...
SnapshotFrame SnapshotCapture::Capture(const SnapshotRequest &request) {
    if (impl_ == nullptr) {
...
bool SnapshotCapture::IsCapturing() const {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
SnapshotInfo SnapshotCapture::GetInfo() const {
    if (impl_ == nullptr) {
        return SnapshotInfo{};
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
