// Streaming API: media stream info, preview URLs, WebRTC signaling, RTSP config.
...
function normalizeMediaStreams(
    response: MediaStreamInfo[] | MediaStreamsResponse,
): MediaStreamInfo[] {
    if (Array.isArray(response)) {
        return response;
    }
...
export function getRtspConfig(init?: ApiRequestOptions): Promise<RtspConfig> {
...
export function getMediaStreams(
    ...
    return requestJson<MediaStreamInfo[] | MediaStreamsResponse>(
        '/api/media/streams',
...
export function getMediaStream(
    ...
    const fallback =
        mockMediaStreams.find((item) => item.stream === stream) ||
        mockMediaStreams[0];
    return requestJson<MediaStreamInfo>(
...
export function getMediaPreviewUrls(
    stream: StreamName,
    init?: ApiRequestOptions,
): Promise<MediaPreviewUrls> {
    return requestJson<MediaPreviewUrls>(
        `/api/media/streams/${stream}/urls`,
        mockMediaPreviewUrls[stream],
        init,
    );
}
...
export async function closeWebrtcPeer(
    peerId: string,
    init?: ApiRequestOptions,
) {
    if (!peerId) {
