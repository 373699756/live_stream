...
bool RegionOverlay::Prepare() {
    ...
    if (state == RegionOverlayState::kInitialized ||
        state == RegionOverlayState::kStarted ||
        state == RegionOverlayState::kStopped) {
        return true;
    }
    ...
    state = RegionOverlayState::kInitialized;
    return true;
}
...
void RegionOverlay::Release() {
    ...
    StopRefreshThread();
    {
        ...
        if (state != RegionOverlayState::kCreated) {
            state = RegionOverlayState::kDeinitialized;
        }
    }
...
void RegionOverlay::RefreshMediaChannels() {
    ...
    media_channels = options.media_channels;
    media_bound = IsValidChannel(media_channels.venc) &&
                  IsValidChannel(media_channels.vpss);
}
...
bool RegionOverlay::Start() {
    bool need_init = false;
    {
        std::lock_guard<std::mutex> lock(mutex);
        need_init = state == RegionOverlayState::kCreated ||
                    state == RegionOverlayState::kDeinitialized;
    }
    ...
    std::lock_guard<std::mutex> lock(mutex);
    if (state == RegionOverlayState::kStarted) {
        return true;
    }
    if (state == RegionOverlayState::kStopped) {
        state = RegionOverlayState::kInitialized;
    }
    if (state != RegionOverlayState::kInitialized) {
        return false;
    ...
    if (!media_bound) {
        const MediaChannels channels = options.media_channels;
...
bool RegionOverlay::BindMedia(const MediaChannels &channels) {
    std::lock_guard<std::mutex> lock(mutex);
    if (state == RegionOverlayState::kStarted) {
        return false;
    }
