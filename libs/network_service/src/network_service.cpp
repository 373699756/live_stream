...
private:
  ...
  bool LoadConfigs() {
    ...
    if (options_.config_service == nullptr) {
      ...
      ConfigJson json = options_.config_service->GetValue(kConfigName);
      if (!json.is_object()) {
        NetworkInterfaceConfig config = DefaultConfig(options_.default_ifname);
        loaded_configs[config.ifname] = config;
        network_json = NetworkJsonWithConfigs(network_json, loaded_configs);
  ...
  bool ApplyToPlatform(const NetworkInterfaceConfig &config) {
    ...
    return platform_->SetGateway(config.ifname, config.gateway) &&
           platform_->SetDnsServers(config.dns_servers);
  }
...
const char *NetworkService::Name() { return "network_service"; }
