...
bool Application::Start(const StartupPaths &paths,
    ...
    if (static_root_override != nullptr && static_root_override[0] != '\0') {
        app_config.static_root = static_root_override;
    }
    app_config_ = app_config;
    Info("app",
         "App config: http=%u rtsp=%u onvif=%u "
         "advertise=%s static_root=%s",
         static_cast<unsigned>(app_config_.http_port),
         static_cast<unsigned>(app_config_.rtsp_port),
         static_cast<unsigned>(app_config_.onvif_device_port),
         app_config_.advertise_host.c_str(),
