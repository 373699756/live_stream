...
function normalizeAlarmInfo(
    ...
    return {
        ...
        last_trigger_time_ms: nonNegativeNumber(
            status?.last_trigger_time_ms,
            0,
        ),
...
function normalizeAlarmRule(
    ...
    return {
        ...
        min_duration_ms: Math.max(
            0,
            Math.round(
                finiteNumber(rule?.min_duration_ms, fallback.min_duration_ms),
            ),
        ),
        repeat_interval_ms: Math.max(
            ...
            Math.round(
                finiteNumber(
                    rule?.repeat_interval_ms,
                    fallback.repeat_interval_ms ?? 0,
                ),
            ),
        ),
