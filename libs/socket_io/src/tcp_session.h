#ifndef LIVE_STREAM_SOCKET_IO_SRC_TCP_SESSION_H_
#define LIVE_STREAM_SOCKET_IO_SRC_TCP_SESSION_H_
...
public:
    TcpSession(SocketIoImpl *socket_io, std::shared_ptr<EventLoop> loop, int fd,
               ConnectionId id, const TcpListenOptions &options,
               TcpCallbacks callbacks, SocketAddress local, SocketAddress peer);
    ~TcpSession();

    bool Start();
    bool Send(const uint8_t *data, size_t size);
    bool SendSlices(const SocketWriteSlices &slices);
    bool Close(TcpCloseReason reason);
    bool CloseAfterSend();
    uint32_t PendingBytes() const;
    SocketConnectionInfo GetInfo() const;
    ConnectionId id() const { return id_; }
    SocketAddress peer() const { return peer_; }
...
private:
    ...
    struct OutBuffer {
        std::array<OutSlice, kMaxSocketWriteSlices> slices{};
        size_t slice_size = 0;
    ...
    bool BuildOutBuffer(const SocketWriteSlices &slices, OutBuffer *buffer) const;
    ...
    // pending_bytes_ 是 socket_io 对慢客户端的统一背压指标，上层协议不应再维护
    // 独立 socket 写队列。
...
#endif  // LIVE_STREAM_SOCKET_IO_SRC_TCP_SESSION_H_
