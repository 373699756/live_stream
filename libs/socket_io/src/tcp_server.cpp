...
TcpServer::TcpServer(SocketIoImpl *socket_io, TcpServerId id,
...
SocketAddress TcpServer::LocalAddress() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return local_;
}
...
void TcpServer::AcceptLoop() {
    while (true) {
        ...
        SocketAddress local = GetSocketAddress(fd, false);
        if (local.port == 0) {
            socket_io_->AddRejected();
            continue;
        }
        ...
        const ConnectionId id = socket_io_->AllocateConnectionId();
        auto connection = std::make_shared<TcpSession>(
            socket_io_, session_loop, accepted.Release(), id, options_,
            callbacks_, local, FromSockAddr(peer_addr));
        if (!socket_io_->AddAcceptedConnection(connection,
                                            options_.max_connections)) {
            socket_io_->AddRejected();
            continue;
        }
        if (!connection->Start()) {
            socket_io_->RemoveConnection(id);
            socket_io_->AddRejected();
            continue;
        }
        // fd 加入 IO loop 前先注册到连接表，后续 read/close 回调到达时，
        // 协议层已经能通过 connection id 查询诊断。
        socket_io_->AddAccepted();
        socket_io_->DispatchAccept(callbacks_, id, connection->peer());
    }
}
