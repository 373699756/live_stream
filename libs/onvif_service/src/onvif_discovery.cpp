...
std::string BuildDefaultEndpointUuid(const OnvifServerOptions &options,
    ...
    if (system_service != nullptr) {
        const DeviceInfo info = system_service->GetDeviceInfo();
...
std::string EndpointUuid(const OnvifServerOptions &options,
    ...
    return BuildDefaultEndpointUuid(options, system_service);
}
...
std::string BuildDiscoveryProbeMatches(
    ...
    const std::string endpoint_uuid = EndpointUuid(options, system_service);
    ...
    const std::string body =
        ...
        "<d:Scopes>" +
        BuildScopes(options, system_service) +
        "</d:Scopes>"
        "<d:XAddrs>" +
