...
size_t EstimatePacketCount(uint32_t size, uint32_t mtu_bytes,
                           VideoCodec codec) {
  if (size + kRtpHeaderSize <= mtu_bytes) {
    return 1;
  }
  const uint32_t max_fragment =
      codec == VideoCodec::kH265 ? mtu_bytes - kH265FuOverhead
                                  : mtu_bytes - kH264FuOverhead;
  if (max_fragment == 0) {
    return 1;
  }
...
std::vector<NalPayload> ParseAnnexBNalPayloads(const uint8_t *data,
                                               size_t size) {
  // Encoders usually deliver Annex-B access units:
  // start code + NAL, start code + NAL... RTP packetization must split those
  // units because each RTP payload carries exactly one NAL or one NAL fragment.
  std::vector<NalPayload> payloads;
  if (data == nullptr || size == 0) {
    return payloads;
  }

  size_t offset = FindAnnexBStartCode(data, size, 0);
  if (offset == std::string::npos) {
    payloads.push_back(NalPayload{data, size});
    return payloads;
  }

  while (offset != std::string::npos) {
    const size_t prefix_size = AnnexBStartCodeSize(data, size, offset);
    if (prefix_size == 0) {
      break;
    }
    const size_t nal_begin = offset + prefix_size;
    const size_t next = FindAnnexBStartCode(data, size, nal_begin);
    size_t nal_end = next == std::string::npos ? size : next;
    while (nal_end > nal_begin && data[nal_end - 1] == 0) {
      --nal_end;
    }
    if (nal_end > nal_begin) {
      payloads.push_back(NalPayload{data + nal_begin, nal_end - nal_begin});
    }
    offset = next;
  }
  return payloads;
...
std::vector<RtpPacket> RtpPacketizer::Packetize(const EncodedFrame &frame,
  ...
  const std::vector<NalPayload> payloads =
      ParseAnnexBNalPayloads(payload, size);
  if (payloads.empty()) {
    return packets;
  }
  ...
  for (size_t i = 0; i < payloads.size(); ++i) {
    PacketizeNal(frame, payloads[i].data,
                 static_cast<uint32_t>(payloads[i].size),
                 i + 1 == payloads.size(), sequence, ssrc, &packets);
  }
  return packets;
}
...
void RtpPacketizer::PacketizeNal(const EncodedFrame &frame,
                                 const uint8_t *payload, uint32_t size,
                                 bool marker, uint16_t *sequence,
                                 uint32_t ssrc,
                                 std::vector<RtpPacket> *packets) const {
  ...
  if (size + kRtpHeaderSize <= mtu_bytes_) {
    SendRtpPacket(frame, payload, size, marker, sequence, ssrc, packets);
    return;
  }
  if (frame.codec == VideoCodec::kH265) {
    PacketizeH265(frame, payload, size, marker, sequence, ssrc, packets);
  } else {
    PacketizeH264(frame, payload, size, marker, sequence, ssrc, packets);
  }
}
...
void RtpPacketizer::PacketizeH264(const EncodedFrame &frame,
  ...
  while (offset < size) {
    ...
    SendRtpPacket(frame, fragment.data(),
                  ...
                  marker && offset + chunk >= size, sequence, ssrc, packets);
    offset += chunk;
    start = false;
  }
}
...
void RtpPacketizer::PacketizeH265(const EncodedFrame &frame,
  ...
  while (offset < size) {
    ...
    fragment.push_back((nal0 & 0x81) | (kH265FuNalType << 1));
