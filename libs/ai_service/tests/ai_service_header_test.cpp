...
int main() {
  if (std::string(live_stream::AiService::StaticName()) != "ai_service") {
    return 1;
  }

  FakeConfigService config_service;
  live_stream::AiServiceOptions options;
  ...
  live_stream::AiService service(options);
  if (!service.Start()) {
    return 2;
  }
  live_stream::AiServiceStats stats = service.GetStats();
  if (!stats.enabled || stats.backend_available) {
    return 3;
  }

  if (!config_service.SetValue("ai", DisabledAiConfig())) {
    return 4;
  }
