#ifndef LIVE_STREAM_SOCKET_IO_SOCKET_IO_H_
#define LIVE_STREAM_SOCKET_IO_SOCKET_IO_H_

#include "event.h"
#include "media/media_buffer.h"

#include <cstddef>
#include <cstdint>
#include <memory>
#include <string>
#include <utility>
#include <vector>

namespace live_stream {

struct SocketAddress {
    std::string ip = "0.0.0.0";
    uint16_t port = 0;
};

using ConnectionId = uint64_t;
using TcpServerId = uint64_t;
using UdpSocketId = uint64_t;

constexpr size_t kMaxSocketWriteSlices = 8;

struct SocketWriteSlice {
    // buffer 为空时 data 会在进入 TCP out buffer 时复制；buffer 非空时 data
    // 可直接指向对应的媒体内存，由发送队列保存 MediaBufferRef 保活。
    const uint8_t *data = nullptr;
    size_t size = 0;
    MediaBufferRef buffer;
};

struct SocketWriteSlices {
    SocketWriteSlice slices[kMaxSocketWriteSlices];
    size_t slice_size = 0;

    bool Add(const uint8_t *data, size_t size,
             MediaBufferRef buffer = MediaBufferRef()) {
        if (size == 0) {
            return true;
        }
        if (data == nullptr || slice_size >= kMaxSocketWriteSlices) {
            return false;
        }
        slices[slice_size].data = data;
        slices[slice_size].size = size;
        slices[slice_size].buffer = std::move(buffer);
        ++slice_size;
        return true;
    }

    size_t TotalSize() const {
        size_t total = 0;
        for (size_t i = 0; i < slice_size; ++i) {
            total += slices[i].size;
        }
        return total;
    }
};

enum class CallbackMode {
    kInlineOnIo,
    kPostToLoop,
};

enum class TcpCloseReason {
    kNormal = 0,
    kRemoteClose,
    kParseError,
    kAuthFailed,
    kQueueFull,
    kPendingLimit,
    kSendStall,
    kReadTimeout,
    kWriteTimeout,
    kInternalError,
};

struct SocketIoOptions {
    uint32_t io_threads = 1;
    uint32_t max_events_per_loop = 64;
    uint32_t task_queue_capacity = 4096;
    bool enable_thread_affinity = false;
    uint32_t first_io_cpu = 0;
    CallbackMode callback_mode = CallbackMode::kInlineOnIo;
    event::Loop *callback_loop = nullptr;
};

struct TcpListenOptions {
    SocketAddress address;
    std::string owner_protocol;
    uint32_t backlog = 128;
    uint32_t max_connections = 64;
    uint32_t send_queue_capacity = 128;
    uint32_t send_buffer_limit_bytes = 1024 * 1024;
    uint32_t send_stall_timeout_ms = 0;
    uint32_t read_timeout_ms = 0;
    uint32_t write_timeout_ms = 0;
    bool reuse_port = false;
    bool tcp_no_delay = false;
    bool keepalive = false;
};

struct UdpBindOptions {
    SocketAddress address;
    uint32_t recv_buffer_bytes = 0;
    uint32_t send_buffer_bytes = 0;
};

using TcpAcceptFn = void (*)(void *user, ConnectionId id, SocketAddress peer);
using TcpReadFn = void (*)(void *user, ConnectionId id, const uint8_t *data,
                           size_t size);
using TcpCloseFn = void (*)(void *user, ConnectionId id,
                            TcpCloseReason reason);
using UdpReadFn = void (*)(void *user, UdpSocketId socket_id, SocketAddress peer,
                           const uint8_t *data, size_t size);

struct TcpCallbacks {
    void *user = nullptr;
    TcpAcceptFn on_accept = nullptr;
    TcpReadFn on_read = nullptr;
    TcpCloseFn on_close = nullptr;
};

struct UdpCallbacks {
    void *user = nullptr;
    UdpReadFn on_read = nullptr;
};

struct SocketIoStats {
    uint32_t active_connections = 0;
    uint64_t accepted_connections = 0;
    uint64_t rejected_connections = 0;
    uint64_t closed_connections = 0;
    uint64_t read_bytes = 0;
    uint64_t written_bytes = 0;
    uint64_t sent_datagrams = 0;
    uint64_t received_datagrams = 0;
    uint64_t send_busy_events = 0;
    uint64_t slow_client_closes = 0;
};

struct SocketConnectionInfo {
    ConnectionId connection_id = 0;
    std::string owner_protocol;
    SocketAddress remote_address;
    SocketAddress local_address;
    uint32_t pending_bytes = 0;
    uint32_t send_queue_length = 0;
    int64_t last_write_at_ms = 0;
    TcpCloseReason close_reason = TcpCloseReason::kNormal;
    bool open = false;
};

const char *TcpCloseReasonName(TcpCloseReason reason);

class ISocketIo {
public:
    virtual ~ISocketIo() = default;

    virtual bool Start() = 0;
    virtual void Stop() = 0;

    virtual event::Loop *DefaultLoop() = 0;
    virtual event::Loop *PickLoop() = 0;

    virtual TcpServerId ListenTcp(event::Loop *loop,
                                  const TcpListenOptions &options,
                                  const TcpCallbacks &callbacks) = 0;
    virtual bool CloseTcp(TcpServerId id) = 0;
    virtual UdpSocketId BindUdp(event::Loop *loop,
                                const UdpBindOptions &options,
                                const UdpCallbacks &callbacks) = 0;
    virtual bool CloseUdp(UdpSocketId id) = 0;

    virtual bool Send(ConnectionId id, const uint8_t *data, size_t size) = 0;
    virtual bool SendSlices(ConnectionId id, const SocketWriteSlices &slices) {
        for (size_t i = 0; i < slices.slice_size; ++i) {
            if (!Send(id, slices.slices[i].data, slices.slices[i].size)) {
                return false;
            }
        }
        return true;
    }
    virtual bool Close(ConnectionId id) = 0;
    virtual bool Close(ConnectionId id, TcpCloseReason reason) {
        (void)reason;
        return Close(id);
    }
    virtual bool CloseAfterSend(ConnectionId id) = 0;
    virtual bool SendTo(UdpSocketId id, SocketAddress address, const uint8_t *data,
                        size_t size) = 0;
    virtual bool SendToSlices(UdpSocketId id, SocketAddress address,
                              const SocketWriteSlices &slices) {
        if (slices.slice_size != 1) {
            return false;
        }
        return SendTo(id, std::move(address), slices.slices[0].data,
                      slices.slices[0].size);
    }
    virtual bool SetUdpPeer(UdpSocketId id, SocketAddress peer) = 0;
    virtual bool SendToPeer(UdpSocketId id, const uint8_t *data,
                            size_t size) = 0;
    virtual bool SendToPeerSlices(UdpSocketId id,
                                  const SocketWriteSlices &slices) {
        if (slices.slice_size != 1) {
            return false;
        }
        return SendToPeer(id, slices.slices[0].data, slices.slices[0].size);
    }

    virtual SocketAddress TcpLocalAddress(TcpServerId id) const = 0;
    virtual SocketAddress UdpLocalAddress(UdpSocketId id) const = 0;
    virtual SocketAddress UdpPeerAddress(UdpSocketId id) const = 0;
    virtual uint32_t PendingBytes(ConnectionId id) const = 0;
    virtual SocketConnectionInfo GetConnectionInfo(
        ConnectionId id) const {
        (void)id;
        return SocketConnectionInfo{};
    }
    virtual std::vector<SocketConnectionInfo>
    ListConnectionInfo() const {
        return std::vector<SocketConnectionInfo>();
    }
    virtual SocketIoStats GetStats() const = 0;
};

std::unique_ptr<ISocketIo> CreateSocketIo(const SocketIoOptions &options);

}  // namespace live_stream

#endif  // LIVE_STREAM_SOCKET_IO_SOCKET_IO_H_
