...
public:
    ...
    StreamFrameSinkId AttachFrameSink(const StreamFrameSinkOptions& options,
                                      IFrameSink* sink) override {
        if (sink == nullptr) {
            return 0;
        }
        const StreamFrameSinkId id = next_sink_id_++;
    ...
    StreamFrameSinkId main_sink_id = 0;
    StreamFrameSinkId sub_sink_id = 0;
...
private:
    StreamFrameSinkId next_sink_id_ = 1;
};
