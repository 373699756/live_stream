...
void OnAccept(void *, live_stream::ConnectionId, live_stream::SocketAddress) {}
...
int main() {
    live_stream::SocketIoOptions options;
    options.io_threads = 2;
    auto engine = live_stream::CreateSocketIo(options);
    if (!engine) {
        return 1;
    }
    if (!engine->Start()) {
