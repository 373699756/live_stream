...
function sharedHiddenTaskConfig(config: AiConfig) {
  ...
  const source =
    draftTaskByName(config, 'object_detection') ?? config.tasks[0] ?? fallback;
  return {
    backend: source.backend,
    stream: source.stream,
    model_path: source.model_path.trim() || fallback.model_path,
    input_width: positiveConfigInteger(source.input_width, fallback.input_width),
    input_height: positiveConfigInteger(source.input_height, fallback.input_height),
    inference_interval_ms: positiveConfigInteger(
      source.inference_interval_ms,
      fallback.inference_interval_ms,
    ),
    max_results: positiveConfigInteger(source.max_results, fallback.max_results),
  };
}
      ...
      <div className="ai-snapshot-list">
        {latestAlerts.length === 0 ? (
          ...
          latestAlerts.map((alert) => {
            const imageUrl = aiAlertImageUrl(alert.image_url, alert.timestamp_ms);
            return (
              <button
                type="button"
              ...
              >
                <img alt={`${taskLabel(alert.task)} ${alert.id}`} src={imageUrl} />
...
export function AiAlertsPage() {
  ...
  const { statuses, playbackUrls } = useLiveView(previewStream);
  const {
    status,
    alarmStatus,
    lastAlarmEvent,
  ...
  useEffect(() => {
    ...
    if (perimeterRegions.length > 0 && activeRegionIndex >= perimeterRegions.length) {
    ...
    (updater: (regions: AiPerimeterRegion[]) => AiPerimeterRegion[]) => {
      updateDraftWith((current) => {
        ...
        return updateTaskConfig(current, 'perimeter_detection', {
          perimeter_regions: updater(task.perimeter_regions ?? []).map(
            normalizeRegion,
          ),
        });
      });
  ...
  const perimeterOverlay = (
    ...
    >
      {videoArea && editingPerimeter ? (
        <div className="ai-perimeter-video-area" style={videoAreaStyle(videoArea)} />
      ) : null}
      {videoArea
        ...
        <main className="ai-console-main">
          ...
          <section className="ai-runtime-compact">
            <div className="ai-runtime-kpis">
              <div>
                <span>事件</span>
                <strong>
                  {draft?.tasks.filter((task) => task.enabled).length ?? 0} 启用
                </strong>
              </div>
              <div>
                <span>后端</span>
              ...
              {orderedTaskStatuses.map((taskStatus, index) => (
                ...
                >
                  <div>
                    <strong>
                      {taskStatus
                        ? taskShortLabel(taskStatus.config.task)
                        : taskShortLabel(kTaskOrder[index])}
                    </strong>
                    <span>
                      {taskStatus ? streamLabel(taskStatus.config.stream) : '--'}
                    </span>
                  </div>
                  <StatusBadge
                  ...
                  />
                  <small>
                    {numberText(taskStatus?.stats.inference_count ?? 0)} 次 /{' '}
                    {numberText(taskStatus?.stats.active_results ?? 0)} 个结果
                  </small>
                </div>
              ))}
            </div>
          ...
          <section className="ai-event-workbench">
            <div className="ai-event-workbench-header">
              ...
              <div className="ai-sensitivity-control" aria-label="AI 灵敏度">
                <span>灵敏度</span>
                <div>
                  {kSensitivityOptions.map((option) => (
                    <button
                      type="button"
                      className={sensitivity === option.value ? 'active' : ''}
                      disabled={!draft}
                      key={option.value}
                      onClick={() => updateSensitivity(option.value)}
                    >
                      {option.label}
                    </button>
                  ))}
                    ...
                    <article className="ai-event-switch-row" key={taskName}>
                      <label className="ai-event-switch-main">
                        <input
                          ...
                          type="checkbox"
                          onChange={(event) =>
                            updateTaskEnabled(taskName, event.target.checked)
                          }
                        />
                        <span>
                          <strong>{taskLabel(taskName)}</strong>
                          <em>{alertCounts[taskName]} 张抓图</em>
                        </span>
                      </label>
                      <StatusBadge
                        state={taskBadgeState(taskStatus)}
