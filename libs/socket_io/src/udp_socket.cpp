...
constexpr const char *kModuleName = "socket_io";

}  // namespace

UdpSocket::UdpSocket(SocketIoImpl *socket_io, UdpSocketId id,
                     const UdpBindOptions &options,
                     const UdpCallbacks &callbacks)
    : socket_io_(socket_io), id_(id), options_(options), callbacks_(callbacks) {}
...
bool UdpSocket::Start(const std::shared_ptr<EventLoop> &loop) {
    ...
    SocketAddress local = GetSocketAddress(fd.get(), false);
    if (local.port == 0) {
        Error(kModuleName,
...
bool UdpSocket::SendTo(SocketAddress address, const uint8_t *data, size_t size) {
...
bool UdpSocket::SendToSlices(SocketAddress address,
                             const SocketWriteSlices &slices) {
    if (slices.slice_size > kMaxSocketWriteSlices) {
        return false;
    }
    std::shared_ptr<EventLoop> loop;
    {
    ...
    for (size_t i = 0; i < slices.slice_size; ++i) {
        const SocketWriteSlice &slice = slices.slices[i];
        if (slice.size == 0) {
            continue;
        }
...
bool UdpSocket::SendToSlicesInLoop(SocketAddress address,
                                   const SocketWriteSlices &slices) {
...
bool UdpSocket::SendToPeerSlices(const SocketWriteSlices &slices) {
    SocketAddress peer;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (!has_peer_) {
...
SocketAddress UdpSocket::PeerAddress() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return has_peer_ ? peer_ : SocketAddress{};
}
...
void UdpSocket::HandleRead() {
    ...
    while (true) {
        ...
        if (n > 0) {
            ...
            // 要求上层当场消费，不得保存指针。
            socket_io_->DispatchUdp(callbacks_, id_, FromSockAddr(peer), buffer,
