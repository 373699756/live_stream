...
void HardwarePipeline::SetConfig(const MediaPipelineConfig& config) {
    config_ = config;
    BuildChannels();
}
...
MediaCapabilities HardwarePipeline::GetCapabilities() const {
    return sdk_->GetCapabilities();
}

bool HardwarePipeline::InitSystem() {
...
bool HardwarePipeline::DeinitSystem() {
    Stop();
...
bool HardwarePipeline::ApplyVencRoi(StreamId stream_id) {
...
bool HardwarePipeline::ApplyImageConfig(const ConfigJson& image_config) {
    return sdk_->ApplyImageConfig(config_, image_config);
}

hisisdk::ExposureInfo HardwarePipeline::QueryExposureInfo() const {
    return sdk_->QueryExposureInfo(config_);
}

void HardwarePipeline::BuildChannels() {
