...
public:
    ...
    bool SendResponse(ISocketIo *socket_io, ConnectionId connection_id,
                      const HttpResponse &response,
                      bool close_after_response) const;
    bool SendResponseSlices(ISocketIo *socket_io, ConnectionId connection_id,
                            const HttpResponse &response,
                            const MediaOutSlice *body_slices,
                            size_t body_slice_size, size_t body_size,
                            bool close_after_response) const;
    bool EnqueueStreamingChunk(ISocketIo *socket_io,
                               ConnectionId connection_id,
                               const uint8_t *data, size_t size) const;
    bool EnqueueStreamingSlices(ISocketIo *socket_io,
                                ConnectionId connection_id,
                                const MediaOutSlice *slices,
                                size_t slice_size) const;
    void CloseConnection(ISocketIo *socket_io, ConnectionId connection_id,
                         TcpCloseReason reason) const;

private:
    bool SendStreamingNetSlices(ISocketIo *socket_io,
                                ConnectionId connection_id,
