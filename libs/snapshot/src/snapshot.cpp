...
Snapshot::Snapshot(const SnapshotConfig &config)
    : Snapshot(
          SnapshotOptions{config, nullptr, nullptr, MediaChannels{},
                                 nullptr}) {}
...
const char *Snapshot::StaticName() { return "snapshot"; }
...
SnapshotFrame Snapshot::Capture(const CaptureRequest &request) {
    if (impl_ == nullptr) {
    ...
    MediaChannels channels;
    {
        ...
        if (impl_->options.device_media != nullptr &&
            !impl_->options.device_media->IsStarted()) {
...
SnapshotStats Snapshot::GetStats() const {
    if (impl_ == nullptr) {
        return SnapshotStats{};
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
    SnapshotStats stats = impl_->stats;
