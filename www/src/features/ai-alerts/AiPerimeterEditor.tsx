...
function normalizeRegion(region: AiPerimeterRegion, index: number): AiPerimeterRegion {
...
export function AiPerimeterEditor({
  ...
  const frameResolution =
    activeStatus?.resolution || videoConfig?.streams[activeStream]?.resolution;
  const frame = useMemo(() => parseResolution(frameResolution), [frameResolution]);
  const videoArea =
      ...
      <div className="ai-perimeter-layout">
        ...
        <aside className="ai-perimeter-controls">
          ...
          <div className="ai-region-source-row">
            ...
            <span>{regions.length > 0 ? `${regions.length} 个区域` : '整幅画面'}</span>
          </div>
          <div className="ai-region-current">
            ...
            {activeRegion ? (
              <em>
                x {formatPercent(activeRegion.x)} / y {formatPercent(activeRegion.y)} / w{' '}
                {formatPercent(activeRegion.width)} / h {formatPercent(activeRegion.height)}
              </em>
            ) : null}
          </div>
          <div className="ai-region-list">
            {regions.length === 0 ? (
              ...
              regions.map((region, index) => (
                <button
                  type="button"
                  className={activeRegionIndex === index ? 'active' : ''}
                  key={`${region.name}-${index}`}
                  onClick={() => {
                ...
                >
                  ...
                  <span>
                    w {formatPercent(region.width)} / h {formatPercent(region.height)}
                  </span>
                </button>
              ))
            )}
          ...
          <div className="ai-region-actions">
            ...
            <button type="button" disabled={regions.length === 0} onClick={clearRegions}>
              清空
            </button>
            <button
              type="button"
          ...
          {saveMessage ? <div className="save-hint">{saveMessage}</div> : null}
          {videoError ? <div className="status-note error-note">{videoError}</div> : null}
        </aside>
      </div>
    </section>
  );
